...
async function FillTracker(username, params = {}) {
    ...
    const config = {
      ...
      trackerwrap: params.thisTracker || $('tag-tab[data-key="#active"]'),
      historywrap: params.thisHistory || $('tag-tab[data-key="#archived"]'),
      commwrap: params.thisCommTracker || $('tag-tab[data-key="#comms"]'),
      socialwrap: params.thisSocialTracker || $('tag-tab[data-key="#socials"]'),
      devwrap: params.thisDevTracker || $('tag-tab[data-key="#dev"]'),
      reqwrap: params.thisDevTracker || $('tag-tab[data-key="#requests"]'),
    ...
    $('#search-topics .tablebasic > tbody > tr', doc).each(function (i, e) {
      if (i > 0) {
        ...
        if (!config.ignoreforums.includes(location) && !config.ignoreforumids.includes(location_id)) {
          ...
          const threadDesc = $(cells[2]).find('.desc').text().replace('[', '<tag-highlight>').replace(']', '</tag-highlight>');
      ...
      const reqhistory = config.reqhistoryforums.includes(location) || config.reqhistoryforumids.includes(location_id);
          ...
          if (history) {
            ...
            config.commwrap.append(formatItem('archived', threadLink, title, threadDesc, location_id, location, lastPosterId, lastPoster, postDate));
          } else if (social) {
            ...
            config.socialwrap.append(formatItem('archived', threadLink, title, threadDesc, location_id, location, lastPosterId, lastPoster, postDate));
          } else if (dev) {
